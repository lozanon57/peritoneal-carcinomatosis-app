...
export function HIPECProtocolCard({ protocol, index = 0 }: HIPECProtocolCardProps) {
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-3 space-y-2">
      ...
      <div className="font-semibold text-gray-900 text-sm">{protocol.drug}</div>
      <div className="grid grid-cols-3 gap-2">
        <div className="flex flex-col items-center gap-1 bg-gray-50 rounded-lg p-2">
          ...
          <span className="text-[10px] text-gray-500">Dose</span>
          <span className="text-xs font-semibold text-gray-800 text-center leading-tight">{protocol.dose}</span>
        </div>
        <div className="flex flex-col items-center gap-1 bg-red-50 rounded-lg p-2">
          <Thermometer size={14} className="text-red-500" />
          <span className="text-[10px] text-gray-500">Temp</span>
          <span className="text-xs font-semibold text-red-700">{protocol.temperature}</span>
        </div>
        <div className="flex flex-col items-center gap-1 bg-blue-50 rounded-lg p-2">
          ...
          <span className="text-xs font-semibold text-blue-700">{protocol.duration}</span>
        </div>
      </div>
      {protocol.notes && (
        <p className="text-xs text-gray-500 leading-relaxed italic">{protocol.notes}</p>
      )}
    </div>
  )
}
